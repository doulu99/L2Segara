	...
	protected void runImpl()
	{
			...
			if (blessedScroll)
			{
				...
				for (final int blessedweaponscroll : BLESSED_WEAPON_SCROLLS)
				{
					if (scroll.getItemId() == blessedweaponscroll)
					{
						...
						minEnchantLevel = Config.BLESSED_ENCHANT_MIN;
						maxEnchantLevel = Config.BLESSED_ENCHANT_MAX;
		...
		synchronized (item)
		{
			...
			else
			{
				if (crystalScroll)
				{
					sm = SystemMessage.sendString("Failed in Crystal Enchant. The enchant value of the item become " );
					activeChar.sendPacket(sm);
				}
