	...
	protected void runImpl()
	{
		...
		try
		{
           ...
           else if (_command.startsWith("antibot"))
           {
			...
			else if (_command.startsWith("Folder "))
			{
				Folder(activeChar, _command.substring(12));
			}
		    else if (this._command.startsWith("welcome_"))
		    {
		        WelcomeHtm.ShowWelcome(activeChar, this._command, null);
		    }			
...
else
{
	...
	private void Folder(final L2PcInstance activeChar, final String path)
	{
		if (path.contains(".."))
			return;
		
		final String filename = "data/" + path;
		final NpcHtmlMessage html = new NpcHtmlMessage(1);
		html.setFile(filename);
		activeChar.sendPacket(html);
	}
