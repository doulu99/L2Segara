...
public final class L2TeleporterInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(final L2PcInstance player, final String command)
	{
		...
		else if (command.startsWith("GrandBoss1"))
		{
			doPartyTeleport(player, 50192);
		}
		else if (command.startsWith("GrandBoss2"))
		{
			doPartyTeleport(player, 50193);
		}
		else if (command.startsWith("GrandBoss3"))
		{
			doPartyTeleport(player, 50194);
		}
		else if (command.startsWith("GrandBoss4"))
		{
			doPartyTeleport(player, 50195);
		}
		else if (command.startsWith("GrandBoss5"))
		{
			doPartyTeleport(player, 50196);
		}
		else if (command.startsWith("GrandBoss6"))
		{
			doPartyTeleport(player, 50197);
		}
		else if (command.startsWith("GrandBoss7"))
		{
			doPartyTeleport(player, 50198);
		}
		else if (command.startsWith("GrandBoss8"))
		{
			doPartyTeleport(player, 50199);
		}
	...
	private static void doPartyTeleport(L2PcInstance player, int val)
	{
	      ...
	      L2ItemInstance i;
	      int itemId = 57; // ITEM ID
	      final L2Party party = player.getParty();
	      L2Object teleporter = player.getLastFolkNPC();
	      for (L2PcInstance p : party.getPartyMembers())
	      {
	         i = p.getInventory().getItemByItemId(itemId);
	         if (i == null || i.getCount() < 1) // ITEM COUNT
	         {
	            player.sendMessage("All party members must have 1 Item before entering this zone."); // EDIT THIS MESSAGE
	            return;
	         }
	      ...
	      if (list.getTeleId() == 50192)
	      {
	         for (final L2PcInstance temp : player.getParty().getPartyMembers())
	         {
	            if (temp.getInventory().getItemByItemId(itemId) != null)
	            {
	               temp.destroyItemByItemId("item", itemId, 1, player, true);
	               temp.teleToLocation(list.getLocX(), list.getLocY(), list.getLocZ(), true);
	            }
	         }
	      }
	      
	      else if (list.getTeleId() == 50193)
	      {
	         for (final L2PcInstance temp : player.getParty().getPartyMembers())
	         {
	            if (temp.getInventory().getItemByItemId(itemId) != null)
