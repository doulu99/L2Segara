...
public final class L2RaidBossInstance extends L2MonsterInstance
{
	...
	public boolean doDie(final L2Character killer)
	{
		...
		if (player != null)
		{
	       // RaidBoss auto noble START by Thug!
	    	   ...
	    	   if (getNpcId() == Config.BOSS_ID) //Barakiel as default.
	    	   {
	    		   if (player.getParty() != null)
	    		   {
	    			   for (L2PcInstance member : player.getParty().getPartyMembers())
	    			   {
	    				   ...
	    				   {
	    					   if (member.isInsideRadius(getX(), getY(), getZ(), Config.RADIUS_TO_RAID, true, true)) //to prevent exploit
	    					   {
	    						   member.setNoble(true);
	    						   player.setNoble(true);
	    						   member.sendMessage("You and your party gained nobless status for defeating " + getName() + "!");
	    						   member.broadcastPacket(new SocialAction(player.getObjectId(), 16)); // Amazing
	    					   }
	               	else
	               	{
