...
public final class L2ArmorSet
{
    ...
    public boolean isEnchanted10(L2PcInstance player)
    {
         // Player don't have full set
        if(!containAll(player))
            return false;
        ...
        if(chestItem.getEnchantLevel() < 10)
            return false;
        if(_legs != 0 && legsItem.getEnchantLevel() < 10)
            return false;
        if(_gloves != 0 && glovesItem.getEnchantLevel() < 10)
            return false;
        if(_head != 0 && headItem.getEnchantLevel() < 10)
            return false;
        if(_feet != 0 && feetItem.getEnchantLevel() < 10)
            return false;
