...
import com.l2jfrozen.gameserver.L2AntiBot;
	...
	public void reduceCurrentHp(final double damage, final L2Character attacker, final boolean awake)
	{
		...
		if (this instanceof L2MonsterInstance)
		{
			...
			master = null;
				...
				if(Config.SHOW_HP_POLY)
				{	
					if (Config.Raid_Boss_Ids_For_Hp.contains(getNpcId()))
					{
						setTitle("HP Left: " + (int) getHpPercent() + "%");
						updateAbnormalEffect();
						
						if (getCurrentHp() * 100 / getMaxHp() >= 99.9)
						{
							switch (Rnd.get(3))
							{
							 case 0:
							 {
								 broadcastPacket(new CreatureSay(getObjectId(), Say2.ALL, getName(), "Who dares to wake me i am " + getName() + " one of the Gods!"));
								 break;
							 }
							 case 1:
							 {
								 broadcastPacket(new CreatureSay(getObjectId(), Say2.ALL, getName(), "Gods!! give me a power for Protect my Life"));
								 break;
							 }
							 case 2:
							 {
								 broadcastPacket(new CreatureSay(getObjectId(), Say2.ALL, getName(), "the war began the end coming soon....."));
								 break;
							 }
						   }
						}
	...
	public boolean doDie(final L2Character killer)
	{
		...
		{	
			if (Config.Raid_Boss_Ids_For_Hp.contains(getNpcId()))
			{
				setTitle("");
				updateAbnormalEffect();
				broadcastPacket(new CreatureSay(getObjectId(), Say2.ALL, getName(), "forgive me master!"));
			}
		}
